...
(n=44) was previously analyzed by Schmidt IM et al. (*Nat Commun.* 2024;15:7368)
...
> ⚠️ SIMULATION NOTICE — All numerical results are [SIMULATED] from a
> synthetic cohort. Replace with real KPMP values after DUA approval and
> pipeline re-run (python3 src/train.py).
...
(KPMP) SomaScan plasma dataset (n=44, biopsy-confirmed kidney disease; DOI:
10.48698/6VRN-ZE53). Arm 1: L2 logistic regression on clinical variables (age,
sex, eGFR). Arm 2: Lasso logistic regression on proteomic features alone. Arm 3
(primary): Lasso feature selection followed by XGBoost classification with
isotonic regression probability calibration via CalibratedClassifierCV. All arms
were validated using repeated stratified 5-fold cross-validation (50 iterations,
random_state=42). Primary metric: log loss. Calibration assessed by Expected
Calibration Error (ECE), Brier score, and reliability diagrams. Reported per
TRIPOD+AI guidelines.

**Results [SIMULATED]:** Clinical-only model AUROC 0.870 [95% CI 0.534–1.000],
log loss 0.463. Proteomics-only collapsed under Lasso regularisation (AUROC
0.500), consistent with n=44 high-dimensional constraints. Combined calibrated
model AUROC 0.858 [0.521–1.000], Brier 0.155. ECE post-isotonic calibration:
0.429 vs 0.393 uncalibrated. All results require replacement with real KPMP data.
...
Kidney Precision Medicine Project (KPMP, n=44), providing strong evidence for the
...
applied to the publicly available KPMP SomaScan dataset (DOI: 10.48698/6VRN-ZE53),
using log loss as the primary optimisation metric and isotonic regression
...
beyond age, sex, and eGFR; (3) compare isotonic-calibrated versus uncalibrated
...
10.48698/6VRN-ZE53), a publicly available credentialed dataset comprising 44
...
Stage 3 CalibratedClassifierCV(method='isotonic', cv=5).
...
### 3.1 Cohort Characteristics [SIMULATED — Table 1]
...
Following variance filtering, 200 SomaScan proteins retained [SIMULATED; actual
KPMP dataset measures ~7,000 proteins]. Full characteristics in Table 1.

### 3.2 Model Performance Comparison [SIMULATED — Table 2]

Results across 50 repeated CV folds are in Table 2. Clinical-only (Arm 1) achieved
AUROC 0.870 [0.534–1.000] and log loss 0.463 [0.227–0.929], confirming that age,
...
Proteomics-only (Arm 2) exhibited null discriminative performance (AUROC 0.500,
log loss 0.693). Lasso C=0.01 was sufficient to drive all protein coefficients to
zero under n=44 high-dimensional conditions. This outcome does not imply absence of
...
Combined calibrated (Arm 3, primary) achieved AUROC 0.858 [0.521–1.000], log loss
...
[SIMULATED]. Wide CIs throughout reflect n=44 and are expected; all estimates are
exploratory.

### 3.3 Calibration Analysis [SIMULATED — Figure 1]

ECE: calibrated 0.429 vs uncalibrated 0.393. Brier: 0.369 vs 0.309. Under
synthetic conditions the calibration benefit of isotonic regression was not
demonstrated, which is mechanistically expected: isotonic calibration improves
...
Under synthetic conditions Lasso selected only eGFR and age. With real KPMP data,
...
Under synthetic conditions, clinical variables dominated discrimination, and
...
sample sizes substantially larger than n=44.
...
(1) n=44 is severely underpowered for high-dimensional prediction modelling; all
